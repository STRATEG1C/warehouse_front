export const api = 'http://70.10.0.101/api';

export const API_LOGIN = '/auth/login';
export const API_LOGOUT = '/auth/logout';
export const API_CURRENT_USER = '/auth/current-user';

export const API_LOCATIONS = '/warehouse/locations';

export const API_SUPPLIES = '/supply/supplies';
export const API_START_UNLOADING = '/supply/start-unloading';
export const API_UNLOAD_PRODUCT = '/supply/supplies/:supplyId/pallet/:palletId';

export const API_STAFF = '/staff/users';

export const API_TASKS = '/staff/tasks';
